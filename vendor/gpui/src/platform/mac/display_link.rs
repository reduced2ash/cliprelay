...
pub struct DisplayLink {
    ...
    // Dev-session fallback: when the CVDisplayLink cannot start (broken
    // display state, e.g. headless/remote sessions), a 16ms timer drives
    // the same dispatch source so the app still renders.
    timer: Option<std::thread::JoinHandle<()>>,
    timer_stop: Option<std::sync::Arc<std::sync::atomic::AtomicBool>>,
}
...
impl DisplayLink {
    pub fn new(
        ...
        unsafe {
            ...
            // Dev-session tolerance: when the CVDisplayLink cannot be
            // created (broken display state, headless sessions), keep the
            // dispatch source so the timer fallback can drive frames.
    ...
    pub fn start(&mut self) -> Result<()> {
        unsafe {
            ...
            let started = self
                .display_link
                .as_mut()
                .map(|link| link.start().is_ok())
                .unwrap_or(false);
            if !started || std::env::var("GPUI_FORCE_TIMER_DISPLAY").is_ok() {
                // Fall back to a fixed-rate timer so the app renders even
                // when the display link cannot tick (broken display state).
                eprintln!("[display-link] CVDisplayLink started={started}; using timer fallback");
    ...
    pub fn stop(&mut self) -> Result<()> {
        ...
        unsafe {
            ...
            if self.display_link.is_some() {
                let _ = self.display_link.as_mut().unwrap().stop();
            }
        }
        Ok(())
    }
