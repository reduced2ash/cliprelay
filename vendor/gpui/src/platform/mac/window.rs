...
impl MacWindow {
    pub fn open(
        ...
        unsafe {
            ...
            // Dev-session fallback: the occlusion notification never fires
            // when the display pipeline is broken, so the display link
            // would never start. Force it at creation instead.
            if show && std::env::var("GPUI_FORCE_TIMER_DISPLAY").is_ok() {
...
impl PlatformWindow for MacWindow {
    ...
    fn start_window_move(&self) {
        let window = self.0.lock().native_window;
        unsafe {
            // Synthesize a left-mouse-down at the current pointer and hand
            // it to performWindowDragWithEvent: so custom title bars can
            // drag the window (macOS 10.11+).
            let location: NSPoint =
                msg_send![class!(NSEvent), mouseLocationOutsideOfEventStream];
            let event: cocoa::base::id = msg_send![class!(NSEvent),
                mouseEventWithType: cocoa::appkit::NSEventType::NSLeftMouseDown
                location: location
...
extern "C" fn window_did_change_occlusion_state(this: &Object, _: Sel, _: id) {
    ...
    unsafe {
        let visible = lock
            ...
            .occlusionState()
            .contains(NSWindowOcclusionState::NSWindowOcclusionStateVisible);
        // Dev-session fallback: when the display pipeline reports the
        // window never visible (broken display state), force the display
        // link anyway so the app still renders (timer fallback inside).
        if visible || std::env::var("GPUI_FORCE_TIMER_DISPLAY").is_ok() {
...
unsafe extern "C" fn step(view: *mut c_void) {
    ...
    if n % 125 == 0 {
        eprintln!("[display-link] step invocations={n}");
    }
    if let Some(mut callback) = lock.request_frame_callback.take() {
        drop(lock);
        callback(Default::default());
