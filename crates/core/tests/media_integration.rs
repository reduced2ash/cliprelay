...
use cliprelay_core::db::{Database, MediaMetadata};
use cliprelay_core::media::{normalize_edit_spec, EditSpec, MediaIndexer, MediaProcessor, ScanResult};
...
impl Fixture {
    fn new() -> Self {
        ...
        let processor = MediaProcessor::new(Arc::clone(&db), export_dir.clone(), cliprelay_core::media::EncoderMode::Software);
        Self {
            dir,
            db,
            indexer,
...
fn manifest_probe_thumbnail_preview_timeline_flow() {
    ...
    assert!(metadata.duration > 2.5 && metadata.duration < 4.0, "duration {}", metadata.duration);
    ...
    assert!(!fixture.db.media_needs_probe(&metadata.path, metadata.size_bytes, metadata.mtime, true).unwrap());

    // Thumbnail: 640x360 fit-decrease JPEG.
    let thumbnail = fixture.indexer.ensure_thumbnail(media_id, None).expect("thumbnail");
    assert!(thumbnail.is_file());
    let row = fixture.db.get_media(media_id).unwrap().unwrap();
    assert_eq!(row.thumbnail_path.as_deref(), Some(thumbnail.to_str().unwrap()));
    ...
    let timeline = fixture.indexer.ensure_timeline(media_id, 12).expect("timeline");
    assert!(timeline.is_file());
    let row = fixture.db.get_media(media_id).unwrap().unwrap();
    assert_eq!(row.timeline_path.as_deref(), Some(timeline.to_str().unwrap()));
...
fn export_trim_produces_valid_mp4_and_records() {
    ...
    let progress: std::sync::Arc<dyn Fn(f64, &str) + Send + Sync> =
        std::sync::Arc::new(|_fraction, _stage| {});
    let result = fixture
        .processor
        .export(
            &row,
...
fn export_size_target_two_pass() {
    ...
    let result = fixture
        .processor
        .export(&row, 0.0, 0.0, "fit_bot", 0.4, &EditSpec::default(), &progress)
        .unwrap();
    assert!(result.path.is_file());
    assert!(result.path.starts_with(&fixture.export_dir), "{} vs {}", result.path.display(), fixture.export_dir.display());
...
fn edit_spec_crop_and_overlays_produce_edited_copy() {
    ...
    let result = fixture
        ...
        .unwrap();
    assert!(result.path.is_file());
    let name = result.path.file_name().unwrap().to_string_lossy().into_owned();
...
fn scan_marks_invalid_missing_and_keeps_present() {
    ...
    fixture.refresh();
    assert_eq!(fixture.db.manifest_paths(&fixture.root.to_string_lossy()).unwrap().len(), 2);

    std::fs::remove_file(&a).unwrap();
    let result = fixture.refresh();
    assert_eq!(result.discovered, 1);
    assert_eq!(result.skipped, 1);
    let row_b = fixture.db.get_media_by_path(&b.to_string_lossy()).unwrap().unwrap();
    assert!(row_b.valid);
    let row_a = fixture.db.get_media_by_path(&a.to_string_lossy()).unwrap().unwrap();
    assert!(!row_a.valid);
}
...
fn passthrough_when_source_fits_target() {
    ...
    let result = fixture
        .processor
        .export(&row, 0.0, 0.0, "fit_bot", 49.0, &EditSpec::default(), &progress)
        .unwrap();
    assert!(!result.generated);
